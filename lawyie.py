...
st.markdown("""
    ...
    <div class="sunverse-logo">☀️ SUNVERSE CORP PRESENTS: LAWYIE ⚖️ (Beta Version) </div>
...
def get_lawyie_response(prompt, task="chat"):
    ...
    try:
        completion = groq_client.chat.completions.create(
            model="llama3-70b-8192", # <--- Powerful, active model for legal tasks",
            messages=[
...
with tabs[3]:
    st.write("### Innovative Features: Legal Document Engine")
    doc_type = st.selectbox("Action", ["Draft a Legal Document", "Review a Contract"])
    doc_text = st.text_area("Provide the details or paste the contract here:")
    
    if st.button("Execute"):
        if doc_text:
            with st.spinner("Processing..."):
                task_type = "draft" if "Draft" in doc_type else "review"
                result = get_lawyie_response(doc_text, task_type)
                st.write(result)
        else:
            st.warning("Please provide details or paste a contract first.")
